...
import math as mt
"""
mp_drawing = mp.solutions.drawing_utils #Hernamienta qu
mp_hands = mp.solutions.hands

with mp_hands.Hands(
    #Configuración del modo de trabajo. True - Imagenes
    static_image_mode=True,
    #Cantadad macxima de manos detectades simultaneamente
    max_num_hands=4,
    min_detection_confidence=0.5) as hands:

    image = cv2.imread("C:/Users/chris/OneDrive/Desktop/V.A/Mano.jpg")
    height, width, _ = image.shape

    #image = cv2.flip(image, 1)
    image_rgb = cv2.cvtColor(image, cv2.COLORR_BGR2RGB)
    results = hands.process(image_rgb)
    #Identificamos si hay manos
    if results.multi_hand_landmarks is not None:
        #Dibujamos los puntos y conexiones
        for hand_landmarks in results.multi_hand_landmarks:

            mp_drawing.draw_landmarks(
                image, hand_landmarks, mp_hands.HANDS_CONNECTIONS,
                mp_drawing.Drawing.DrawingSpec(color=(204,0,0), thickness=4, circle_radius=5),
                mp_drawing.Drawing.DrawingSpec(color=(204,0,0), thickness=4)
            )

cv2.imshow("Esqueletizacion", image)
cv2.waitKey(0)
